...
RUN yum install -y wget tar sed

#intall java
RUN mkdir -p /opt/oracle/java && cd /opt/oracle/java \
...
#install tomcat
RUN mkdir -p /opt/apache/tomcat/ && cd /opt/apache/tomcat/ \
...
#&& sed -i 's/\(<Engine name="Catalina".*\)>/\1 jvmRoute="Tomcat"\>/' /opt/apache/tomcat/current/conf/server.xml \
